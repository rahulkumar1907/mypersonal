...
const updateReview = async function(req, res) {
    try {
        ...
        //no books found
        if (!oldReview) {
            return res.status(404).send({ status: false, data: "review not found" });
        }
        ...
        const updatedReview = await reviewModel.findByIdAndUpdate({ _id: reviewId }, { $set: { reviewedAt: date, review, reviewedBy, rating } }, { new: true });
